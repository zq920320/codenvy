 ...
 * and its supplierBY_REFs and may be covered by U.S. and Foreign Patents,
...
r1 = FOREACH s5 GENERATE s4::tmpWs AS tmpWs, s4::delta AS delta, s4::factoryUrl AS factoryUrl, s4::referrer AS referrer, s4::auth AS auth, 
...
r2 = GROUP r1 BY tmpWs;
result = FOREACH r2 {
    t = FOREACH r1 GENERATE TOTUPLE(TOTUPLE(delta / 60), TOTUPLE(factoryUrl), TOTUPLE(referrer), TOTUPLE(auth), TOTUPLE(conv));
    GENERATE group, t;
}
