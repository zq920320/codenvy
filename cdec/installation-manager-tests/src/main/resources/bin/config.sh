...
PREV_CODENVY4_VERSION=$(echo ${AVAILABLE_CODENVY_VERSIONS} | sed 's/.*"\([4-5][^"]*\)","[4-5][^"]*".*\]/\1/')
LATEST_CODENVY4_VERSION=$(echo ${AVAILABLE_CODENVY_VERSIONS} | sed 's/.*"\([4-5][^"]*\)".*/\1/')
...
SINGLE_CODENVY4_RHEL_WITH_ADDITIONAL_NODES_VAGRANT_FILE="../vagrant/single-codenvy4-with-additional-nodes/RHEL7/Vagrantfile"
