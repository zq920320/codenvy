...
public class FactoryWorkspaceIdEnvironmentInitializationFilterTest {
    ...
    public void testEnv() throws ServerException, NotFoundException, IOException, ServletException {
        //given
        when(cache.getById(eq("id-23423"))).thenReturn(usersWorkspace);
        when(usersWorkspace.getId()).thenReturn("wsid-123123");

        Mockito.doAnswer(invocation -> {
            Assert.assertEquals(EnvironmentContext.getCurrent()
                                                  .getWorkspaceId(), "wsid-123123");
            return null;
        })
               .when(chain)
