...
public class BraintreeCreditCardDaoImplTest {
    ...
    public void shouldBeAbleToRegisterFirstCard() throws Exception {
        ...
        verify(eventService).publish(argThat(new ArgumentMatcher<Object>() {
            ...
            public boolean matches(Object o) {
                CreditCardRegistrationEvent event = (CreditCardRegistrationEvent)o;
                return CreditCardRegistrationEvent.EventType.CREDIT_CARD_ADDED.equals(event.getType())
    ...
    public void shouldBeAbleToGetCards() throws Exception {
        ...
        list.add(creditCard);
        when(customer.getCreditCards()).thenReturn(list);
        List<com.codenvy.api.creditcard.shared.dto.CreditCard> result = dao.getCards(ACCOUNT_ID);
    ...
    public void shouldBeAbleToRemoveCards() throws Exception {
        ...
        verify(eventService).publish(argThat(new ArgumentMatcher<Object>() {
            ...
            public boolean matches(Object o) {
                CreditCardRegistrationEvent event = (CreditCardRegistrationEvent)o;
                return CreditCardRegistrationEvent.EventType.CREDIT_CARD_REMOVED.equals(event.getType())
                       && event.getAccountId().equals(ACCOUNT_ID);
            }
        }));
    }
