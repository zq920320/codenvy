...
r1 = UNION R, D;
r2 = FOREACH r1 GENERATE ws, REGEX_EXTRACT(user, '.*@(.*)', 1) AS domain, dt, delta;
r3 = FILTER r2 BY domain != '';
r4 = GROUP r3 BY domain;
result = FOREACH r4 {
    r5 = FOREACH r3 GENERATE TOTUPLE(TOTUPLE(ws), TOTUPLE(domain), TOTUPLE(dt), TOTUPLE(delta));
    GENERATE group, r5;
}
