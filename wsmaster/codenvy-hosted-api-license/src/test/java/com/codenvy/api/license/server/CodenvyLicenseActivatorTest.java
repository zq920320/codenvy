...
public class CodenvyLicenseActivatorTest {
    ...
    public void setUp() throws IOException {
        when(codenvyLicense.getLicenseText()).thenReturn(LICENCE_TEXT);

        codenvyLicenseActivator = new CodenvyLicenseActivator(codenvyLicenseStorage, PUBLIC_KEY, PRODUCT_ID.toCharArray());
    }

    @Test
    public void shouldNotThrowExceptionIfActivationNotRequired() throws Exception {
        when(codenvyLicense.isActivationRequired()).thenReturn(false);
    ...
    public void shouldNotThrowExceptionIfCodenvyActivationLicenseTextExists() throws Exception {
        when(codenvyLicense.isActivationRequired()).thenReturn(true);
        when(codenvyLicenseStorage.loadActivatedLicense()).thenReturn(LICENSE_ACTIVATION_TEXT);

        codenvyLicenseActivator.validateActivation(codenvyLicense);
    }

    @Test(expectedExceptions = LicenseNotActivatedException.class)
    public void shouldThrowExceptionIfActivationTextInvalid() throws Exception {
        when(codenvyLicense.isActivationRequired()).thenReturn(true);
        when(codenvyLicenseStorage.loadActivatedLicense()).thenReturn("some invalid activation text");
    ...
    public void shouldThrowExceptionIfActivationTextRequiresActivation() throws Exception {
        when(codenvyLicense.isActivationRequired()).thenReturn(true);
        when(codenvyLicenseStorage.loadActivatedLicense()).thenReturn(LICENCE_TEXT);
