...
public class TestScriptUsersCreatedFromFactory extends BaseTest {
    ...
    public void setUp() throws Exception {
        ...
        events.add(
                Event.Builder.createFactoryUrlAcceptedEvent("tmp-1", "fact1", "ref1").withDate("2013-01-01").build());
        events.add(
                Event.Builder.createFactoryUrlAcceptedEvent("tmp-2", "fact1", "ref1").withDate("2013-01-01").build());
    ...
    @Test
    public void testScriptByFactoryUrl() throws Exception {
        MapStringSetValueData valueData =
                (MapStringSetValueData)executeAndReturnResult(ScriptType.SET_USER_CREATED_FROM_FACTORY_BY_FACTORY_URL,
                                                              log,
                                                              context);

        assertEquals(valueData.size(), 1);
        assertEquals(valueData.getAll().get("fact1").getAll().size(), 1);
        assertTrue(valueData.getAll().get("fact1").getAll().contains("user4@gmail.com"));
    }
}
