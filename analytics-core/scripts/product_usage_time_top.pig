...
---------------------------------------------------------------------------
a1 = LOAD '$resultDir/$entity/$interval' USING PigStorage() AS (user: chararray, count: long, time: long);
a2 = GROUP a1 ALL;
...
-- Calculate for 1D
---------------------------------------------------------------------------
d1 = LOAD '$resultDir/$entity/P1D' USING PigStorage() AS (user: chararray, count: long, time: long);
...
-- Calculate for 7D
---------------------------------------------------------------------------
d7 = LOAD '$resultDir/$entity/P7D' USING PigStorage() AS (user: chararray, count: long, time: long);
...
-- Calculate for 30D
---------------------------------------------------------------------------
d30 = LOAD '$resultDir/$entity/P30D' USING PigStorage() AS (user: chararray, count: long, time: long);
...
-- Calculate for 60D
---------------------------------------------------------------------------
d60 = LOAD '$resultDir/$entity/P60D' USING PigStorage() AS (user: chararray, count: long, time: long);
...
-- Calculate for 90D
---------------------------------------------------------------------------
d90 = LOAD '$resultDir/$entity/P90D' USING PigStorage() AS (user: chararray, count: long, time: long);
...
-- Calculate for 365D
---------------------------------------------------------------------------
d360 = LOAD '$resultDir/$entity/P365D' USING PigStorage() AS (user: chararray, count: long, time: long);
...
---------------------------------------------------------------------------
dlt = LOAD '$resultDir/$entity/P100Y' USING PigStorage() AS (user: chararray, count: long, time: long);
