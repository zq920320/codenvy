...
public class SystemLicenseFilterTest {
    ...
    public void shouldSkipRequestByFilter() throws Exception {
        //given
        when(servletRequest.getRequestURI()).thenReturn("/api/license/legality");
    ...
    public void setHasFairSourceLicenseAccepted(boolean hasFairSourceLicenseAccepted) throws Exception {
        ...
        when(requestFactory.fromUrl(API_ENDPOINT + "/license/legality")).thenReturn(request);
    ...
    private void setIsAdminAndNotInteractive(boolean isAdmin, boolean noUserInteraction) {
        ...
        when(subject.hasPermission(SystemDomain.DOMAIN_ID, null, MANAGE_CODENVY_ACTION)).thenReturn(isAdmin);
    }
