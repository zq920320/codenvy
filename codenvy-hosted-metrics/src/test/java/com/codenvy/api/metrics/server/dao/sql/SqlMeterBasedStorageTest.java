...
import com.codenvy.api.AbstractSQLTest;
import com.codenvy.api.metrics.server.MemoryUsedMetric;
import com.codenvy.api.metrics.server.UsageInformer;
import com.codenvy.api.metrics.server.dao.sql.SqlMeterBasedStorage.SQLUsageInformer;
...
public class SqlMeterBasedStorageTest extends AbstractSQLTest {
    ...
    public void shouldBeAbleToStoreMetric()
        ...
        assertEquals(meterBasedStorage.getMetric(((SQLUsageInformer)usageInformer).getRecordId()), expected);
    }
    ...
    public void shouldBeAbleToUpdateEndTime()
        ...
        MemoryUsedMetric expected =
                new MemoryUsedMetric(1000,
                                     metricPeriod.getCurrent().getStartDate().getTime(),
                                     metricPeriod.getCurrent().getStartDate().getTime() + 1000,
                                     "usr-123",
                                     "ac-46534",
                                     "ws-235423",
                                     "run-234");
        ...
        MemoryUsedMetric actual =
                meterBasedStorage.getMetric(((SQLUsageInformer)usageInformer).getRecordId());

        assertTrue(new Date().getTime() < actual.getStopTime() + 1000 * 60);
        assertEquals(actual.getStartTime().longValue(), metricPeriod.getCurrent().getStartDate().getTime());
    ...
    public void shouldNotUpdateAfterStop()
        ...
        //when
        long recordId = ((SQLUsageInformer)usageInformer).getRecordId();
        MemoryUsedMetric expected = meterBasedStorage.getMetric(recordId);
        usageInformer.resourceInUse();
        //
        assertEquals(meterBasedStorage.getMetric(recordId).getStopTime(), expected.getStopTime());
    }
    ...
    public void shouldGetSumByDifferentWs()
        ...
        assertEquals(result.get("ws-124"), 0.290133);
        assertEquals(result.get("ws-235423"), 0.217867);
