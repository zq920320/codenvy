...
public class TestWorkspacesCreated extends BaseTest {
    ...
    public void testExecute() throws Exception {
        ...
        ListListStringValueData value = (ListListStringValueData)executeAndReturnResult(ScriptType.WORKSPACES_CREATED, log, params);
        List<ListStringValueData> all = value.getAll();

        assertEquals(all.size(), 3);
        assertTrue(all.contains(new ListStringValueData(Arrays.asList("ws1", "user1"))));
        assertTrue(all.contains(new ListStringValueData(Arrays.asList("ws2", "user2"))));
        assertTrue(all.contains(new ListStringValueData(Arrays.asList("ws3", "user2"))));
    }
}
