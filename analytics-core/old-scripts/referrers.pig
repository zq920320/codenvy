...
g1 = GROUP f BY referrer;
g = FOREACH g1 {
    -- count of created workspaces (we are interested in unique names only)
    t1 = FOREACH f GENERATE ws;
    t2 = DISTINCT t1;

    GENERATE *, COUNT(t2) AS wsCount;
}

result = FOREACH g GENERATE group, TOBAG(SUM(f.delta), wsCount, COUNT(f), SUM(f.auth), SUM(f.conv), SUM(f.bld), SUM(f.run), SUM(f.dpl));
