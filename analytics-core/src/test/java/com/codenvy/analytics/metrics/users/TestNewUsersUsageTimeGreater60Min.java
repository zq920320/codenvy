...
public class TestNewUsersUsageTimeGreater60Min extends BaseTest {
    ...
    public void prepare() throws Exception {
        ...
        events.add(Event.Builder.createSessionUsageEvent(UID1, "ws", "s1", false).withDate("2014-01-01").withTime("10:05:00").build());

        events.add(Event.Builder.createSessionUsageEvent(UID1, "ws", "s2", false).withDate("2014-01-01").withTime("11:00:00").build());
        events.add(Event.Builder.createSessionUsageEvent(UID1, "ws", "s2", false).withDate("2014-01-01").withTime("11:05:00").build());

        events.add(Event.Builder.createSessionUsageEvent(UID1, "ws", "s3", false).withDate("2014-01-02").withTime("10:00:00").build());
        events.add(Event.Builder.createSessionUsageEvent(UID1, "ws", "s3", false).withDate("2014-01-02").withTime("11:10:00").build());

        events.add(Event.Builder.createSessionUsageEvent(UID2, "ws", "s5", false).withDate("2014-01-02").withTime("11:00:00").build());
        events.add(Event.Builder.createSessionUsageEvent(UID2, "ws", "s5", false).withDate("2014-01-02").withTime("12:10:00").build());
    ...
    public void testGetValue() throws Exception {
        Metric metric = MetricFactory.getMetric(MetricType.NEW_USERS_USAGE_TIME_GREATER_60_MIN);
        ...
        // user1 70min, but wasn't created today => 0
        // user2 70min => 1
