...
public class TestActiveSubscribersRecipientGroup extends BaseTest {
    ...
    public void testIsNotActiveSubscriberWhenTariffPlanIsWrong() throws Exception {
        ...
        assertFalse(group.isActiveSubscriber(account, builder.build()));
    }
    ...
    public void testActiveSubscriberIfSubscriptionPeriodBelow() throws Exception {
        doReturn(ActiveSubscribersRecipientGroup.TARIFF_MANAGED_FACTORY).when(account)
                                                                        .getAttribute(
                                                                                ActiveSubscribersRecipientGroup
                                                                                        .TARIFF_PLAN);
        ...
        doReturn("" + Utils.parseDate("20131009").getTimeInMillis()).when(account)
                                                                    .getAttribute(
                                                                            ActiveSubscribersRecipientGroup
                                                                                    .TARIFF_END_TIME);
        builder.put(Parameters.TO_DATE, "20131010");
        assertTrue(group.isActiveSubscriber(account, builder.build()));
    }
