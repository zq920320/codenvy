    ...
    public void onEvent(CreditCardRegistrationEvent event) {
        switch (event.getType()) {
            case CREDIT_CARD_ADDED:
                LOG.info("EVENT#credit-card-added# USER#{}# ACCOUNT#{}#", event.getUserId(), event.getAccountId());
                break;

            case CREDIT_CARD_REMOVED:
                LOG.info("EVENT#credit-card-removed# USER#{}# ACCOUNT#{}#", event.getUserId(), event.getAccountId());
                break;
            default:
        }
    }
